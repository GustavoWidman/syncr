...
use super::structure::{ClientConfig, ConfigStructure, ModeConfig, ServerConfig};
use anyhow::bail;

#[derive(Debug)]
pub struct Config {
    path: PathBuf,
    cached: ConfigStructure,
}
...
impl Config {
    ...
    pub fn as_ref(&self) -> &ConfigStructure {
        &self.cached
    }

    pub fn as_mut_ref(&mut self) -> &mut ConfigStructure {
        &mut self.cached
    }
    ...
    fn new(path: PathBuf) -> Result<Self, anyhow::Error> {
        ...
        let config = Self {
            path,
            cached: ConfigStructure::default(),
        };
